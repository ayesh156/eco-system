...
🌐 {{shopWebsite}}`;
    ...
    if (supplierIdMatch && (method === 'PUT' || method === 'PATCH')) {
      ...
      const existing = await db.supplier.findUnique({ where: { id: supplierIdMatch[1] } });
    ...
    if (supplierIdMatch && method === 'DELETE') {
      ...
      if (existing._count.grns > 0) {
        await db.supplier.update({ where: { id: supplierIdMatch[1] }, data: { isActive: false } });
        return res.status(200).json({ success: true, message: 'Supplier deactivated (has GRNs)' });
      }

      await db.supplier.delete({ where: { id: supplierIdMatch[1] } });
    ...
    if (path === '/api/v1/grns' && method === 'POST') {
      ...
      for (let i = 0; i < items.length; i++) {
        const item = items[i];
        if (!item.productId) {
          return res.status(400).json({ success: false, message: `Item ${i + 1}: Product ID is required` });
        }
        ...
        if (typeof item.costPrice !== 'number' || item.costPrice < 0) {
          return res.status(400).json({ success: false, message: `Item ${i + 1}: Valid cost price is required` });
        }
      }
      ...
      if (supplier.shopId !== shopId) {
        return res.status(403).json({ success: false, message: 'Supplier does not belong to your shop' });
      }
      
      const subtotal = items.reduce((sum: number, item: any) => sum + (item.quantity * item.costPrice), 0);
      const totalAmount = subtotal + tax - discount;
      
      const count = await db.gRN.count({ where: { shopId } });
    ...
    if (grnIdMatch && method === 'PUT') {
      ...
      const grn = await db.gRN.findUnique({ where: { id: grnIdMatch[1] } });
      if (!grn) return res.status(404).json({ success: false, message: 'GRN not found' });
      if (grn.shopId !== shopId) return res.status(403).json({ success: false, message: 'Access denied' });
    ...
    if (grnPaymentsMatch && method === 'GET') {
      ...
      const payments = await db.gRNPayment.findMany({
        where: { grnId: grn.id },
        orderBy: { sentAt: 'desc' },
        include: {
    ...
    if (grnPaymentMatch && method === 'POST') {
      ...
      const grn = await db.gRN.findUnique({ where: { id: grnId } });
      if (!grn) return res.status(404).json({ success: false, message: 'GRN not found' });
      if (grn.shopId !== shopId) return res.status(403).json({ success: false, message: 'Access denied' });
  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: white; padding: 32px; border-radius: 0 0 16px 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
      ...
      <div style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); border-radius: 12px; padding: 20px; margin-bottom: 24px; border: 1px solid #e2e8f0;">
        ...
        <p style="color: #0f172a; font-size: 16px; font-weight: 600; margin: 0 0 4px 0;">${grn.supplier.name}</p>
    ...
    // Generate GRN PDF
    const grnPDFMatch = path.match(/^\/api\/v1\/grns\/([^/]+)\/pdf$/);
    if (grnPDFMatch && method === 'GET') {
      const shopId = getEffectiveShopId(req, query);
      if (!shopId) return res.status(403).json({ success: false, message: 'Unauthorized' });

      const grnIdOrNumber = grnPDFMatch[1];
      
      // Find GRN with all necessary relations
      const grn = await db.gRN.findFirst({
        where: {
          OR: [
            { id: grnIdOrNumber },
      ...
      // Import PDF service (lazy load for performance)
      const { generateGRNPDF } = await import('../src/services/pdfService');
      ...
      const pdfData = {
        ...
        expectedDeliveryDate: grn.expectedDeliveryDate?.toISOString() || grn.date.toISOString(),
        ...
        paymentMethod: grn.paymentMethod || undefined,
        items: grn.items.map((item: any) => ({
          productName: item.product?.name || item.productName || 'Unknown Product',
          category: item.product?.category?.name || undefined,
          unitPrice: Number(item.unitPrice || item.costPrice),
          ...
          totalAmount: (item.acceptedQuantity || item.quantity || 0) * Number(item.unitPrice || item.costPrice),
        ...
        totalOrderedQuantity: grn.items.reduce((sum: number, item: any) => sum + (item.orderedQuantity || item.quantity ||  0), 0),
        totalReceivedQuantity: grn.items.reduce((sum: number, item: any) => sum + (item.receivedQuantity || item.quantity || 0), 0),
        totalAcceptedQuantity: grn.items.reduce((sum: number, item: any) => sum + (item.acceptedQuantity || item.quantity || 0), 0),
        totalRejectedQuantity: grn.items.reduce((sum: number, item: any) => sum + (item.rejectedQuantity || 0), 0),
        subtotal: Number(grn.subtotal),
        totalDiscount: grn.totalDiscount ? Number(grn.totalDiscount) : undefined,
        discountAmount: Number(grn.discountAmount || grn.discount || 0),
        taxAmount: Number(grn.taxAmount || grn.tax || 0),
    ...
    // GET reminders for a GRN
    const grnRemindersMatch = path.match(/^\/api\/v1\/grns\/([^/]+)\/reminders$/);
    if (grnRemindersMatch && method === 'GET') {
      const shopId = getEffectiveShopId(req, query);
      if (!shopId) return res.status(403).json({ success: false, message: 'Unauthorized' });

      const grnIdOrNumber = grnRemindersMatch[1];
      ...
      const reminders = await db.gRNReminder.findMany({
        where: { grnId: grn.id },
        orderBy: { sentAt: 'desc' }
      });
    ...
    // POST create reminder for a GRN
    if (grnRemindersMatch && method === 'POST') {
      const shopId = getEffectiveShopId(req, query);
      if (!shopId) return res.status(403).json({ success: false, message: 'Unauthorized' });

      const grnIdOrNumber = grnRemindersMatch[1];
      const { type, channel, message, supplierPhone, supplierName } = body;
      ...
      const reminder = await db.gRNReminder.create({
        data: {
          grnId: grn.id,
          shopId,
          type: type || 'PAYMENT',
      ...
      const reminderCount = await db.gRNReminder.count({
        where: { grnId: grn.id }
      });
