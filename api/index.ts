...
// VERSION: 2026-02-10-v1 (Added GRN Reminders)
...
const API_VERSION = '2026-02-10-v1';
  ...
  <div class="container">
    ...
    if (path === '/api/v1/invoices' && method === 'POST') {
      try {
        ...
        // ==================== DEDUCT STOCK ====================
        // After successful invoice creation, deduct stock for each item
...
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f1f5f9;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    if (grnPDFMatch && method === 'GET') {
      const effectiveShopId = getEffectiveShopId(req, query);
      if (!effectiveShopId) return res.status(403).json({ success: false, message: 'Unauthorized' });
      ...
      // Import PDF service from backend (lazy load for performance)
      const pdfModule = await import('../backend/src/services/pdfService');
      const { generateGRNPDF } = pdfModule;
